...
Bootstrap with current Flutter platform templates:
...
CI generates the Android runner, grants network/microphone permissions, runs format/analyze/tests, and builds a debug APK. Non-loopback bridge URLs must use HTTPS, and bridge bearer credentials are never sent to direct ChatGPT/CDN asset URLs.

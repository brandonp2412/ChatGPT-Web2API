...
class ChatGPTDom:
    ...
    async def click_send(self) -> None:
        """Activate the send button through the browser's native click path.
        ...
        try:
            point = json.loads(result) if result != "sent" else None
        except (TypeError, json.JSONDecodeError):
            point = None
        if point is not None:
            await d._cdp("Input.dispatchMouseEvent", {
    ...
    async def _capture_selector_diagnostic(self, selector_name: str) -> None:
        ...
        failures are diagnosable without SLOPPA_DIAGNOSE=1. Called at the point of

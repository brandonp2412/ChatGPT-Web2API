...
Design (agreed in collaborative review):
  - File at ``~/.sloppa/owned_tabs.json``, keyed by ``instance_id``.
  - ``instance_id`` derived from configured identity (chrome profile + cdp
    port + server/transport identity), overridable via ``SLOPPA_INSTANCE_ID``.
...
class TabRegistry:
    ...
    def derive_instance_id(
        ...
        """Stable per-configuration identity, or the SLOPPA_INSTANCE_ID override.
        ...
        vs ``release-notes``).
        """
        override = os.environ.get("SLOPPA_INSTANCE_ID", "").strip()
        if override:
            return override

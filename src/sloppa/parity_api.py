...
from .api_server import MODEL_MAP, APIServer
...
class ParityAPIServer(APIServer):
    ...
    async def _handle_conversations(self, request: web.Request) -> web.Response:
        ...
        try:
            try:
                ...
                return self._bad_request("offset and limit must be integers")
            if offset < 0 or limit < 1:
                return self._bad_request("offset must be non-negative and limit must be positive")
            offset = min(offset, 2_000_000_000)
            limit = min(100, limit)
            order = request.query.get("order", "updated")
            if order not in {"updated", "created"}:
                return self._bad_request("order must be 'updated' or 'created'")
            raw_items = await self._driver.get_conversations(
                offset=offset, limit=limit, order=order
            )
            items = raw_items if isinstance(raw_items, list) else []
            if raw_items is not items:
                logger.warning(
                    "conversation_list driver returned unexpected type=%s; using empty list",
                    type(raw_items).__name__,
                )
            logger.info(
                "conversation_list offset=%d limit=%d returned=%d order=%s",
                offset,
                limit,
                len(items),
                order,
            ...
            return web.json_response(
                {"object": "list", "data": items, "offset": offset, "limit": limit}
            )
    ...
    async def _handle_conversation_search(self, request: web.Request) -> web.Response:
        ...
        if len(query) > 500:
            return self._bad_request("query must be 500 characters or fewer")
        try:
    ...
    async def _handle_conversation(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            logger.info(
                "conversation_projection id=%s raw_nodes=%d projected_messages=%d "
                "projected_tree_nodes=%d truncated=%s omitted=%d",
                request.match_info["conversation_id"],
                len(mapping) if isinstance(mapping, dict) else 0,
                len(data["messages"]),
                len(data["tree"]),
                data["messages_truncated"],
                data["messages_omitted"],
            )
    ...
    async def _handle_rich_send(self, request: web.Request) -> web.Response:
        ...
        logger.info(
            "Rich send started request_id=%s active=%d",
            request.headers.get("X-Request-ID", "none"),
            self._active_send_count,
        )
        ...
        try:
            ...
            logger.info(
                "Rich send finished request_id=%s active=%d duration_ms=%d",
                request.headers.get("X-Request-ID", "none"),
                self._active_send_count,

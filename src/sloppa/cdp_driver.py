...
class CDPDriver:
    ...
    async def navigate_new_chat(self, gizmo_id: str = None) -> None:
        ...
        # ChatGPT can retain the prior conversation in SPA state while the
        # address bar is at ``/``. Activate the actual New chat control so the
        # composer and conversation route are reset together.
        new_chat_point = await self._js(
            "(function(){"
            "  var links = document.querySelectorAll('a[href=\"/\"]');"
            "  for (var i = 0; i < links.length; i++) {"
            "    if (/new chat/i.test((links[i].innerText || '').trim())) {"
        ...
        try:
            point = json.loads(new_chat_point) if new_chat_point else None
    ...
    async def _verify_send_acknowledged(self) -> bool | None:
        ...
        while _time.monotonic() < deadline:
            try:
                ...
                # These are independent React updates. On a fresh chat the
                # user bubble can be committed before the composer reset (or
                # vice versa), so retain each positive observation across
                # polls instead of requiring both in one snapshot.
                user_message_seen = user_message_seen or current_count > pre_send_count
                composer_cleared_seen = composer_cleared_seen or bool(state.get("composerEmpty"))
                if user_message_seen and composer_cleared_seen:
                    return True
            except Exception:
                pass
            await asyncio.sleep(0.5)

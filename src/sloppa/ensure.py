...
``sloppa ensure`` is a thin one-liner for ZCode hooks: it makes REST
...
from sloppa.cross_process_lock import LockAcquisitionError
...
# / ``SLOPPA_ENSURE_*`` env (see EnsureConfig). Module constants stay as the
...
class _StartupLock:
    ...
    def __init__(self, sse_port: int, timeout: float = _LOCK_TIMEOUT) -> None:
        self._path = str(Path.home() / ".sloppa" / f"sse-startup-{sse_port}.lock")

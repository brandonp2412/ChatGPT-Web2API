"""MCP Server — expose Sloppa as an MCP server for AI agents.
...
Run:
    sloppa-mcp                         # stdio (default)
    sloppa-mcp --transport sse          # SSE on port 8090
    sloppa-mcp --transport sse --port 3000

Prerequisites:
    Run 'sloppa' first to start Chrome with an authenticated session.
"""
...
#                  SLOPPA_ENABLE_WRITE=1.
#   DESTRUCTIVE  — irreversible deletes. Hidden unless
#                  SLOPPA_ENABLE_DESTRUCTIVE=1.
#
...
WRITE_ENV = "SLOPPA_ENABLE_WRITE"
DESTRUCTIVE_ENV = "SLOPPA_ENABLE_DESTRUCTIVE"

# Tools requiring SLOPPA_ENABLE_WRITE=1 to be visible/callable
...
# Tools requiring SLOPPA_ENABLE_DESTRUCTIVE=1 (irreversible account changes)
...
def build_tools() -> list[mcp_types.Tool]:
    ...
    Filters :func:`_build_tools` by the current environment:
      ...
      - Write tools require ``SLOPPA_ENABLE_WRITE=1``.
      - Destructive tools require ``SLOPPA_ENABLE_DESTRUCTIVE=1``.
...
def create_server() -> Server:
    ...
    async def call_tool(
        ...
        if _driver is None:
            raise ConnectionError("Not connected to Chrome. Run 'sloppa' first.")
...
def _mcp_server_identity(config: Config, transport: str, port: int) -> str:
    ...
    ``SLOPPA_INSTANCE_ID`` as the highest-priority override.
    """
    ...
    if config.chatgpt.mcp_session_pool_enabled:
        ...
        try:
            ...
            logger.error(
                ...
                "Run 'sloppa' first to start Chrome. Error: %s",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="sloppa-mcp",
        description="MCP server for Sloppa",
    )

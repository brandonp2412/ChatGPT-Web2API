...
_MAX_CLIENT_BLOCK_TEXT = 20000
_MAX_INITIAL_TRANSCRIPT_MESSAGES = 200


def _client_text(value: Any) -> str:
    text = _string(value) or ""
    if len(text) <= _MAX_CLIENT_BLOCK_TEXT:
        return text
    return text[:_MAX_CLIENT_BLOCK_TEXT] + "\n\n[Output truncated by bridge: too large for client]"


_CLIENT_METADATA_KEYS = {
...
def client_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    """Retain only small metadata needed by the native client.

    ChatGPT attaches large tool/research/debug payloads to every tree node.
    Returning those opaque blobs made a normalized conversation balloon to
    tens of megabytes and caused desktop clients to run out of memory.
    """
    return {
        key: value
        for key, value in metadata.items()
        if key in _CLIENT_METADATA_KEYS and isinstance(value, (str, int, float, bool, type(None)))
    }
...
def normalize_conversation(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    total_branch_messages = len(branch_ids)
    if len(branch_ids) > _MAX_INITIAL_TRANSCRIPT_MESSAGES:
        branch_ids = branch_ids[-_MAX_INITIAL_TRANSCRIPT_MESSAGES:]
    messages = []
    ...
    return {
        ...
        "messages": messages,
        "messages_truncated": total_branch_messages > len(messages),
        "messages_omitted": max(0, total_branch_messages - len(messages)),
        "tree": tree,
...
def normalize_message(
    ...
    *,
    ...
    safe_metadata = client_metadata(metadata)
...
def content_blocks(
    ...
    *,
    ...
    if content_type == "code":
        ...
        output = _client_text(_string(content.get("result")) or _content_text(content))
        blocks.append(
            {
        ...
        blocks.append(
            {
                ...
                "value": {
                    ...
                    "text": _content_text(content)[:10000],
                },
                "metadata": safe_metadata,
            }
        )
...
def _append_parts(blocks: list[dict[str, Any]], content: dict[str, Any]) -> None:
    ...
    for part in _as_list(content.get("parts")):
        if isinstance(part, str):
            if part:
                blocks.append({"type": "text", "text": _client_text(part)})
            continue
...
def _part_to_block(part: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if kind == "execution_output":
        return {
            "type": "execution_output",
            "text": _client_text(part.get("text") or part.get("result")),
            "value": {
                "content_type": kind,
                "text": _client_text(part.get("text") or part.get("result")),
            },
    ...
    if kind == "code":
        return {
            "type": "code",
            "code": _client_text(part.get("text") or part.get("code")),
            "language": part.get("language"),
        }
    text = part.get("text")
    if isinstance(text, str) and text:
        return {"type": "text", "text": _client_text(text)}
    ...
    if kind:
        return {"type": "structured", "content_type": kind, "value": {"content_type": kind}}
    return None

...
class APIServer:
    ...
    def __init__(
        ...
        self.app.middlewares.append(self._cors)
        self.app.middlewares.append(self._request_context)
        self.app.on_response_prepare.append(self._cors_prepare)
    ...
    async def _cors(self, request: web.Request, handler):
        """Allow browser clients served from loopback during local development."""
        ...
        if allowed:
            self._set_cors_headers(origin, response)
        return response

    @staticmethod
    def _set_cors_headers(origin: str, response: web.StreamResponse) -> None:
    ...
    async def _cors_prepare(self, request: web.Request, response: web.StreamResponse) -> None:
        origin = request.headers.get("Origin", "")
        parsed = urlparse(origin)
        if (
            parsed.scheme in {"http", "https"}
    ...
    @web.middleware
    async def _request_context(self, request: web.Request, handler):
        request_id = uuid.uuid4().hex[:12]
        started = time.monotonic()
        try:
            response = await handler(request)
        except Exception:
            logger.exception("request_failed id=%s method=%s path=%s", request_id, request.method, request.path)
            raise
        response.headers["X-Request-ID"] = request_id
        logger.info(
            "request_complete id=%s method=%s path=%s status=%d elapsed_ms=%d",
            request_id,
            request.method,
            request.path,
            response.status,
    ...
    async def _handle_health(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {
                "status": status,
                "ready_for_requests": status in ("starting", "healthy"),
                "chrome_running": chrome_running,
                "cdp_connected": driver_connected,
                "driver_connected": driver_connected,
                "requests_served": self._request_count,
                "active_sends": getattr(self, "_active_send_count", 0),
